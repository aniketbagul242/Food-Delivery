...
const Cart = () => {
  ...
  <button onClick={()=>navigate("/order")} className='border-none text-white bg-red-600 w-56 pt-3 pb-3 pr-0 pl-0 rounded cursor-pointer'>PROCEED TO CHRCKOUT</button>
        </div>
        <div  className='flex-1 hidden sm:block' id='promo'>
          <div>
            <p className='text-gray-600'>If you have a promo code, Enter it here</p>
            <div className='mt-2.4 flex justify-between items-center bg-gray-200 rounded' id='input'>
              <input className='bg-transparent border-none outline-none pl-2.5' type="text" placeholder='promocode' />
              <button className='w-[10vw] pt-3 pb-3 pr-1 pl-1 bg-black border-none text-white rounded'>Submit</button>
            </div>
          </div>
        </div>
      </div>
